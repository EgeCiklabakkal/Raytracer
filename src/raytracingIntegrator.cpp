...
void RaytracingIntegrator::render(Image* img, const Camera* cam,
...
void RaytracingIntegrator::raytrace_routine(const RaytracingIntegrator* integrator,
...
void RaytracingIntegrator::raytrace_singleSample(const RaytracingIntegrator* integrator,
						const Scene* scene, const Camera* cam, Image* img,
						SafeStack<std::pair<float, float>>* pixels)
{
	std::pair<float, float> currPixel;
...
rgb RaytracingIntegrator::rayColor(const Ray& r, int recursion_depth,
...
rgb RaytracingIntegrator::backgroundColor(const Vec2& ij, const Vec3& direction) const
{
...
rgb RaytracingIntegrator::ambientColor(const HitRecord& record) const
{
...
rgb RaytracingIntegrator::diffuseColor(const Ray& r, const HitRecord& record,
...
rgb RaytracingIntegrator::specularColor(const Ray& r, const HitRecord& record,
					const SampleLight& slight) const
{
	rgb I(slight.radiance);
...
rgb RaytracingIntegrator::reflectionColor(const Ray& r, const HitRecord& record,
...
rgb RaytracingIntegrator::refractionColor(const Ray& r, const HitRecord& record,
						int recursion_depth, const Tonemap& tonemap) const
{
...
bool RaytracingIntegrator::handleTexture(HitRecord& record, DecalMode& decal_mode, rgb& color) const
{
	if(!record.texture)
	{
		// has no texture
...
bool RaytracingIntegrator::handleTonemap(const Tonemap& tonemap, HitRecord& record, rgb& color) const
{

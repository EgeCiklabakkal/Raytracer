...
bool MeshTriangle::boundingBox(float time0, float time1, BBox& _box, float epsilon) const
{
        Vec3 vx(a().x(), b().x(), c().x());
        Vec3 vy(a().y(), b().y(), c().y());
        Vec3 vz(a().z(), b().z(), c().z());

        float xmin = vx.minComponent() - epsilon; 
        float ymin = vy.minComponent() - epsilon; 
        float zmin = vz.minComponent() - epsilon; 

        float xmax = vx.maxComponent() + epsilon;
        float ymax = vy.maxComponent() + epsilon;
        float zmax = vz.maxComponent() + epsilon;
